...
class Conv(nn.Module):
    def __init__(self, inp_dim, out_dim, kernel_size=3, stride = 1, bn = False, relu = True):
        ...
        self.conv = nn.Conv2d(inp_dim, out_dim, kernel_size, stride, padding=(kernel_size-1)//2, bias=True).cuda()
        self.relu = None
        self.bn = None
        if relu:
            self.relu = nn.ReLU().cuda()
        if bn:
            self.bn = nn.BatchNorm2d(out_dim).cuda()
...
class Residual(nn.Module):
    def __init__(self, inp_dim, out_dim, Conv_method = Conv):
        ...
        self.conv2 = Conv_method(int(out_dim/2), int(out_dim/2), 3, relu=False)
